...
class PartnerController extends AbstractController
{
    ...
    public function update(Partner $entity, Request $request, EntityManagerInterface $entityManager, SerializerInterface $serializer): Response
    {
        ...
        if ($updatedEntity->getName()) {
            $entity->setName($updatedEntity->getName());
        }
        if ($updatedEntity->getType()) {
            $entity->setType($updatedEntity->getType());
        }
        if ($updatedEntity->getImageName()) {
            $entity->setImageName($updatedEntity->getImageName());
        }
        if ($updatedEntity->getImageFile()) {
            $entity->setImageFile($updatedEntity->getImageFile());
        }
